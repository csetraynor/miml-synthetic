...
produce_MCAR <- function(data, perc.missing, idx.incomplete){ 
  ...
  if (nb_initNA != 0){
    idx_newNA[,which(idx.incomplete==1)][!idx_initNA[,which(idx.incomplete==1)]] <- (runif(n*p.incomp - nb_initNA) <= (perc.missing*(1+nb_initNA/(p.incomp*n))))
  } else {
...
## helper function for DGM inputs
prepare_ann_input <- function(x, new_na) {
  # input: x mice input
  # returns: inputs for DGM model
  ## Collect non-missing data
  x_nonmis <- x[,x %>% summarise_all(~ sum(is.na(.))) %>% unlist() == 0, drop = FALSE] 
  assertthat::assert_that(ncol(x_nonmis) > 0)
  
  ## Collect missing data
  x_mis <- x[,x %>% summarise_all(~ sum(is.na(.))) %>% unlist() > 0, drop = FALSE]
  
  ## Collect numeric data
  x_num <- x_mis %>% select_if(is.numeric)
  assertthat::assert_that(ncol(x_num) > 0) 
  
  ## Collect categorical data
  x_cat <- x_mis %>% select_if(is.factor)
  
  ## Format categorical data
  Mask = bind_cols( x_num, x_cat) %>%
    mutate_all( ~ as.integer(!is.na(.)))
  mod_cat <- model.matrix(~ ., data = model.frame(~ ., x_cat, na.action=na.pass), 
                          contrasts.arg = lapply(x_cat, contrasts, contrasts=FALSE))[,-1,drop=F] %>% as_tibble()
  
  ## Mergen input, must be: binary + numeric + categorical + non_missing
  x_input = bind_cols( x_num, mod_cat, 
                       model.matrix(~ ., data = x_nonmis)[,-1,drop=F] %>%
                         as_tibble())
  
  ## save metadata
  cat_sizes = x_cat %>% summarise_all(~ length(levels(.))) %>% unlist
  num_dim = ncol(x_num)
  cat_dim = ncol(mod_cat)
  ## read var names
  var_names <- c(bind_cols(x_num, x_cat) %>% colnames())
  cat_vars <- gsub(paste(paste0("^", colnames(x_cat)), collapse = "|"), "", colnames(mod_cat))
  
  return(nlist(x_input, num_dim, cat_dim, cat_sizes, Mask, var_names, cat_vars))
}
